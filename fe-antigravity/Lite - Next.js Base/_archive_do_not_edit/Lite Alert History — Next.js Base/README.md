...
This implementation now contains the shared **App Shell**, **Lite / Institution Overview**, and **Lite / Institution Detail** screens built from the supplied visual references and the frozen API contract.
...
- `/lite/alert-history` — placeholder for the next supplied design
- `/lite/alert-review` — placeholder for the next supplied design
...
- Final `Confirm & Send` remains reserved for the Alert Review screen.
...
## Notification flow
Final notification delivery will use the frozen contract's `POST /notifications/simulate` endpoint from Alert Review. The UI must display the returned `simulated: true` semantics and must never imply a real SMS/WhatsApp send.
