...
import { localTime, peakTime, reliabilityNote } from "@/lib/ui/format";
          ...
          <section className="rounded-2xl border border-orange-200 bg-orange-50/60 p-5">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="mt-1 text-[16px] font-extrabold leading-tight text-slate-800">{institution.type === "hospital" ? "Operational readiness may need to be adjusted." : "Outdoor activities may need to be adjusted."}</p>
              </div>
            </div>
          </section>
        </div>
        ...
        <section className="mt-3 rounded-2xl border border-blue-200 bg-blue-50/65 px-5 py-4">
          ...
          {sent && (
            <p className="mt-3 rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-[10px] font-semibold text-emerald-700">
              Sent to: {institution.name} admin contact — {localTime(sent.sent_at, institution.country)} · {sent.channel.toUpperCase()} · simulated: {String(sent.simulated ?? true)}.
            </p>
          )}
        </section>
