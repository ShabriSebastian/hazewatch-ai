...
export async function loadProAlertHistoryData() {
  const base = await loadProInstitutionDetailData();

  if (base.mode === "mock") {
    return {
      ...base,
      historyEvents: proMockHistoryEvents,
    } as const;
  }

  const historyEvents = base.alertHistory.map((alert) => ({
    id: alert.alert_id,
    timestamp: alert.triggered_at,
    status: "alert" as const,
    title: alert.status === "resolved" ? "Alert resolved" : "Forecast alert issued",
    description: `Forecast upper-band peak reached ${alert.forecast_peak_pm25.toFixed(1)} µg/m³ with ${alert.lead_time_hours}h warning lead time.`,
