 ...
 *
 * Pro's horizon must equal the backend's alert horizon
 * (`config.FORECAST_HORIZON_HOURS = 24`). `/alerts` takes no `horizon_hours`
 * parameter and is always evaluated over 24 hours, while the Live Monitor takes
 * its Alert badges from that endpoint and its numbers from `/forecast`. At 12
 * every row rendered an ALERT badge beside a peak below the 35.5 threshold,
 * because nothing in this scenario breaches inside 12 hours - the alerts are all
 * generated by hours 13-24. It also made the 17-24h warning lead times the demo
 * is built on impossible to display.
 */
