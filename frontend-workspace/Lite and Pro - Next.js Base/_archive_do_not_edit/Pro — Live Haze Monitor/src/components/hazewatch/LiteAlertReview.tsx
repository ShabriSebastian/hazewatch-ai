...
function previewMessage(alert: Alert, institution: Institution) {
  const window = peakWindow(alert, institution);
  ...
  if (institution.type === "hospital") {
    return [
      ...
      `The highest impact is expected around ${window}.`,
      firstAction
  ...
  return [
    `HAZE ALERT`,
    `Air quality around ${institution.name} is expected to worsen this afternoon.`,
    `The highest impact is expected around ${window}.`,
    firstAction
...
function ReliabilityIndicator({ forecast }: { forecast: Forecast }) {
  if (!forecast.uncertainty) {
    return (
      <div className="flex max-w-[330px] items-start gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-[10px] text-slate-600">
    ...
    return (
      <div className="flex max-w-[330px] items-start gap-2 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-[10px] text-amber-800">
        ...
        <div>
          ...
          <p className="mt-0.5 text-amber-700">Part of the 24-hour outlook is outside the model's usual training range.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex max-w-[330px] items-start gap-2 rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-[10px] text-emerald-800">
      <ShieldCheck size={14} className="mt-0.5 flex-none" />
      <div>
        <p className="font-extrabold">Forecast is within the model's usual operating range.</p>
...
function EmptyReview({ institutionName }: { institutionName: string }) {
  return (
    <AppShell activePage="alert-review">
      <main className="min-w-0 px-6 py-5 lg:px-7 lg:py-6">
        ...
        <p className="mt-1 text-xs text-slate-500">Review prepared alerts before sending them to your institution's verified contact.</p>
        ...
        <section className="mt-5 rounded-2xl border border-emerald-200 bg-emerald-50 p-7">
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="mt-2 text-sm text-slate-600">{institutionName} is currently in an informational Safe or Watch state. Confirm &amp; Send is available only when the forecast reaches Alert.</p>
...
export function LiteAlertReview() {
  ...
  useEffect(() => {
    loadLiteOverviewData().then(setData).catch((err: unknown) => {
      setError(err instanceof Error ? err.message : "Failed to load Alert Review.");
    });
  }, []);
  ...
  if (!data) {
    return (
      <AppShell activePage="alert-review">
        <main className="p-8 text-sm text-slate-500">Loading alert review…</main>
      </AppShell>
    );
  }
...
function ActiveAlertReview({ data, alert }: { data: ScreenData; alert: Alert }) {
  ...
  async function doConfirm() {
    ...
    setSendError(null);
    try {
      const result = await confirmLiteNotification({
      ...
      setSendError(err instanceof Error ? err.message : "Could not create the simulated notification.");
    } finally {
      setSending(false);
    }
  }
    ...
    <AppShell activePage="alert-review">
      <main className="min-w-0 px-6 py-5 lg:px-7 lg:py-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <h2 className="text-[31px] font-extrabold tracking-tight text-ink">Alert Review</h2>
            <p className="mt-1 text-xs text-slate-500">Review the prepared alert before sending it to your institution's verified contact.</p>
          </div>
          ...
          <section className="rounded-2xl border border-red-200 bg-red-50/65 p-5">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="mt-2 text-[17px] font-extrabold leading-tight text-slate-800">Air quality is expected to worsen this afternoon.</p>
              </div>
            </div>
          </section>
          ...
          <section className="rounded-2xl border border-blue-200 bg-blue-50/55 p-5">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="mt-1 text-[17px] font-extrabold text-slate-800">Verified institution contact</p>
          ...
          <section className="rounded-2xl border border-orange-200 bg-orange-50/60 p-5">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="mt-1 text-[16px] font-extrabold leading-tight text-slate-800">{institution.type === "hospital" ? "Operational readiness may need to be adjusted today." : "Outdoor activities may need to be adjusted this afternoon."}</p>
              </div>
            </div>
          </section>
        </div>
            ...
            <div className="mt-5 rounded-[26px] border-[5px] border-slate-800 bg-[#eee8dc] shadow-sm">
              <div className="flex items-center gap-3 rounded-t-[20px] bg-white px-5 py-4">
                ...
                <div>
                  ...
                  <p className="text-[9px] text-slate-400">Preview · not yet sent</p>
                </div>
              </div>
              <div className="p-8">
                <div className="max-w-[92%] rounded-2xl bg-white p-4 text-[10px] leading-5 text-slate-700 shadow-sm">
                  ...
                  <p className="mt-3 font-bold text-slate-600">Updated: {localTime(alert.triggered_at, institution.country)}</p>
                </div>
              </div>
            </div>
            <p className="mt-4 flex items-center gap-1.5 text-[10px] text-slate-500"><Info size={12} /> This alert is prepared but not yet sent.</p>
          </section>
          ...
          <div className="space-y-3">
            ...
            <section className="rounded-2xl border border-red-200 bg-red-50/50 p-5">
              ...
              <dl className="mt-3 divide-y divide-red-100 text-[10px]">
                ...
                <div className="flex justify-between gap-5 py-2.5"><dt className="text-slate-500">Message status</dt><dd className="font-extrabold text-slate-700">{sent ? "Confirmed · simulated delivery" : "Prepared for verified institution contact"}</dd></div>
              </dl>
        ...
        <section className="mt-3 rounded-2xl border border-blue-200 bg-blue-50/65 px-5 py-4">
          ...
          {sendError && <p className="mt-3 rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-[10px] font-semibold text-red-700">{sendError}</p>}
          {sent && (
            <p className="mt-3 rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-[10px] font-semibold text-emerald-700">
              Simulation recorded as {sent.status}. Sent to: {institution.name} admin contact · {sent.channel.toUpperCase()} · simulated: {String(sent.simulated ?? true)}.
            </p>
          )}
        </section>
        ...
        <p className="mt-2 text-[9px] text-slate-400">MVP note: authentication is not implemented. In production, Confirm &amp; Send should be restricted to authorized institution staff.</p>
      </main>
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/45 p-4" role="dialog" aria-modal="true" aria-labelledby="confirm-title">
          <div className="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-6 shadow-2xl">
            <div className="flex items-start justify-between gap-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs leading-5 text-slate-600">This will create a simulated notification for the verified contact at {institution.name}. No external message will be delivered.</p>
                </div>
              </div>
            ...
            <div className="mt-5 rounded-xl border border-slate-200 bg-slate-50 p-3 text-[11px] text-slate-600">
              ...
              <p className="mt-1"><strong>Recipient:</strong> Verified institution contact</p>
              <p className="mt-1"><strong>Prototype:</strong> Simulated delivery only</p>
            </div>

            {sendError && <p className="mt-3 text-xs font-semibold text-red-600">{sendError}</p>}

            <div className="mt-6 flex justify-end gap-2">
              <button type="button" onClick={() => setConfirmOpen(false)} disabled={sending} className="rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-xs font-extrabold text-slate-700 disabled:opacity-50">Cancel</button>
              <button type="button" onClick={doConfirm} disabled={sending} className="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-2.5 text-xs font-extrabold text-white disabled:opacity-60">
                <Send size={14} /> {sending ? "Confirming…" : "Confirm & Send"}
              </button>
            </div>
          </div>
        </div>
