...
import { loadProInstitutionDetailData } from "@/lib/data/source";
import { ProAppShell } from "./ProAppShell";

type ScreenData = Awaited<ReturnType<typeof loadProInstitutionDetailData>>;
type ViewStatus = "safe" | "watch" | "alert";

function peakTriggerValue(forecast: Forecast) {
  return forecast.peak.pm25_upper ?? forecast.peak.pm25;
}

function statusFromForecast(forecast: Forecast): ViewStatus {
  const value = peakTriggerValue(forecast);
  if (value >= 35.5) return "alert";
...
function warningWindow(alert: Alert | null, forecast: Forecast) {
  const crossed = alert?.threshold_crossed_at ?? forecast.forecast.find((point) => (point.pm25_upper ?? point.pm25) >= 35.5)?.timestamp;
...
function alertHeroCopy(institution: Institution, status: ViewStatus) {
  ...
  return institution.type === "hospital" ? "High haze impact expected" : "High haze impact expected";
}
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-1 text-[10px] font-bold">
          <span className="rounded-lg border border-slate-200 px-3 py-1.5 text-slate-400">6 Hours</span>
          <span className="rounded-lg bg-blue-600 px-3 py-1.5 text-white">12 Hours</span>
          <span className="rounded-lg border border-slate-200 px-3 py-1.5 text-slate-400">24 Hours</span>
        </div>
      </div>
...
export function ProInstitutionDetail() {
  ...
  const { health, institution, forecast, alertResponse, alertHistory, hotspotSummary } = data;
  const alert = alertResponse.alert;
  const status = statusFromForecast(forecast);
  const peak = peakTriggerValue(forecast);
  const reliability = reliabilitySummary(forecast);
  const warningLead = alert?.lead_time_hours ?? forecast.forecast.find((p) => (p.pm25_upper ?? p.pm25) >= 35.5)?.lead_hours ?? null;
  const actions = status === "alert" ? (alert?.recommended_actions ?? []) : [];
  const forecastWindow = warningWindow(alert, forecast);

  return (
    <ProAppShell activePage="institutions" scopeLabel="Institution View" forecastLabel="Next 12 Hours">
      <main className="min-w-0 bg-white px-5 py-5 lg:px-7">
        <div className="mx-auto max-w-[1500px]">
          ...
          <div className="mt-3 grid gap-3 xl:grid-cols-[1.05fr_.72fr_1.08fr]">
            ...
            <RecentAlerts alertHistory={alertHistory} currentStatus={status} />
          </div>
