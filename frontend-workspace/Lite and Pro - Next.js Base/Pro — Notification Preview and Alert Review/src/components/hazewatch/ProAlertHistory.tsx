...
function RiskTrend({ events }: { events: ProHistoryEvent[] }) {
  ...
  const y355 = top + ((max - ALERT_THRESHOLD_PM25) / (max - min)) * plotH;
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-4">
      ...
      <p className="mt-2 text-[9px] text-slate-400">Safe ≤{GOOD_MAX_PM25} · Watch {GOOD_MAX_PM25 + 0.1}–{ALERT_THRESHOLD_PM25 - 0.1} · Alert ≥{ALERT_THRESHOLD_PM25} µg/m³, the same for every institution type. Alerting uses the upper prediction band.</p>
    </section>
  );
}
...
export function ProAlertHistory() {
  ...
  useEffect(() => {
    setError(null);
    loadProAlertHistoryData(institutionId).then((loaded) => {
      setData(loaded);
