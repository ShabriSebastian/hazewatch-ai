...
- `/pro/live-monitor` — Regional Live Haze Monitor
- `/pro/institutions` — Institution Detail
- `/pro/alert-history` — Alert History (interactive status filter + selected-event details)
- `/pro/notification-preview` — placeholder for the next approved screen

The Alert History screen uses contract-backed alert records in API mode. Safe/Watch transition rows only exist in the swappable demo fixture because the frozen Alerts API exposes alert records, not a separate Safe/Watch state-history endpoint.

## Pro Notification Preview / Alert Review

`/pro/notification-preview` is now implemented from the approved Pro visual with contract-safe behavior:

- Alert-only action state. Safe and Watch do not show `Confirm & Send`.
- Delivery target is a verified institution contact only.
- Channel choices come from `Institution.contact_channels`.
- Preview language comes from `Institution.languages`.
- Forecast trigger peak uses the Alert payload (`forecast_peak_pm25`, derived from the upper prediction band).
- A compact reliability indicator appears next to the final confirmation action and is driven by `Forecast.uncertainty.any_point_beyond_training_range`; no invented High/Medium/Low confidence tier is used.
- `Confirm & Send` calls `POST /notifications/simulate` in API mode. Mock mode creates an in-memory simulated Notification with the same UI semantics.
- No real SMS or WhatsApp delivery is implied.
